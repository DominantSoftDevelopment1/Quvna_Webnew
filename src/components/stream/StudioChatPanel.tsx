...
export function StudioChatPanel({
  ...
  return (
    <div
      className={`flex h-full max-h-full min-h-0 min-w-0 flex-col overflow-hidden bg-[#111216] text-white ${className}`}
    >
      {/* Header */}
      <div className="flex h-11 shrink-0 items-center justify-between border-b border-[#303039] bg-[#1f1f23] px-4">
        ...
        <button type="button" className="text-zinc-400 hover:text-white" aria-label="Yana">
          ⋮
        </button>
      </div>
      ...
      {/* Socket hint */}
      {socketHint ? (
        <p className="shrink-0 border-b border-[#303039] px-4 py-2.5 text-[11px] text-amber-400/90">
          {socketHint}
        </p>
      ) : null}
      ...
      <div className="shrink-0 border-b border-[#24262d] bg-[#17181d] px-4 py-3">
        <div className="flex min-w-0 flex-col gap-1 sm:flex-row sm:items-center sm:justify-around sm:gap-3">
          <span className="min-w-0 truncate text-[13px] font-semibold text-zinc-200">
            👑 dava_black 🎁 305
          </span>
          <span className="min-w-0 truncate text-[13px] font-semibold text-zinc-200">
            🥈 wolfking0 🎁 225
      ...
      {pinnedTrimmed ? (
        <div className="shrink-0 border-b border-[#24262d] bg-[#111216] px-4 py-3">
          <div className="rounded-[8px] border border-[#9147ff]/70 bg-[#191322] px-3 py-2.5">
            <p className="mb-1 text-[11px] font-black uppercase text-[#bf94ff]">
              📌 Zakreplangan habar
            </p>
        ...
        <div className="shrink-0 border-t border-red-500/20 bg-red-500/10 px-4 py-2.5">
          ...
          {onDismissError ? (
            <button
              ...
              className="mt-1 w-full text-[11px] text-red-300 underline"
            >
              Yopish
            </button>
          ) : null}
      ...
      <div className="shrink-0 border-t border-[#24262d] px-3 pb-3 pt-3">
        <div className="rounded-[10px] border-2 border-[#9147ff] bg-[#181a20] px-3 py-2.5">
          <div className="flex min-w-0 items-center gap-2">
            <button
              type="button"
              className="flex h-8 w-8 shrink-0 items-center justify-center text-zinc-400 hover:text-zinc-200"
              aria-label="Sovg'a"
            >
            ...
            <span
              className="flex h-8 w-8 shrink-0 select-none items-center justify-center text-[20px] text-zinc-400"
              aria-hidden
            >
              ☺
            </span>
        ...
        <div className="mt-2.5 flex items-center justify-between gap-2 px-1">
          <span className="text-sm text-zinc-400">
            ♦ 0 <span className="mx-1 text-zinc-600">|</span>◯ {liveUserCount}
          </span>
          <button
            type="button"
            onClick={onSend}
            className="shrink-0 rounded-full bg-[#9147ff] px-5 py-1.5 text-[13px] font-bold text-white transition hover:brightness-110"
          >
            Yuborish
          </button>
        </div>
